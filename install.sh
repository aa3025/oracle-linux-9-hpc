...
if [ "$1" = "-h" ]; then
    ...
    echo "You have to supply the location if the OL9 dvd ISO file see `basename $0 -h`"
    exit 0
elif [ ! -z "$1" ]; then
...
yum install -y mc nano net-tools nfs-utils dhcp-server tftp httpd openssh-server firewalld tftp-server git syslinux shim syslinux-tftpboot vsftpd opensm pdsh infiniband-diags # xinetd -y &&
...
sh scripts/firewall.sh $eth_int $eth_ext
...
cp $installpath/EFI/BOOT/*.EFI /tftpboot/ &&
cp /boot/efi/EFI/redhat/shimx64*.efi /tftpboot/ &&
